...
Context-only entries, including prepared summaries and upstream compaction bodies, are reached
through their history events; they are not added to the conversation transcript by indexing. The
provider-switch handoff (or its placeholder) is the exception: it is recorded as message history
...
OR. An empty filter includes messages and events. `since` later than `until` is rejected.

Times are HistoryRecord timestamps in Unix milliseconds, with inclusive `since` and exclusive
`until`. Every history record has a timestamp. Message creation time on `Entry` may differ from
its history time, for example while waiting in the queue. Ordering and cursors use sequence
numbers, so equal or non-monotonic timestamps do not lose rows.

The first page captures an exclusive history upper bound. Later pages retain that bound and seek
by sequence, in oldest-first or newest-first order; a cursor whose order differs from the
request's is rejected. New appends do not displace existing pages. The limit is caller-controlled
(`HistoryPageRequest` default 50, must be positive), independent of the storage cache's page
size.

## Search and expansion

`search_history(HistorySearch, HistoryFilter)` returns top matches with history references,
message/event type, tool name, snippets and FTS5 BM25 scores (smaller sorts first). `HistorySearch`
is `{ text, mode: terms | substring, limit }` with `terms` and limit 20 by default; empty text is
rejected. `terms` requires all whitespace-separated
...
shorter than three characters scan the filtered indexed text, return newest-first without a score,
and report `used_text_index: false`. Short substring case folding follows SQLite `lower()` (ASCII);
longer searches use the FTS tokenizer.
...
protocol replay units. Code that reconstructs model messages from them must validate tool
pairing and reasoning replay requirements separately.

Text indexes include message text, readable reasoning (`plaintext`, else `display`), and tool
name plus argument/result JSON. Image blocks, reasoning signatures/ciphertext, upstream opaque
bodies, and streamed delta payloads are not indexed. Search-tool arguments/results remain
filterable/readable but are excluded from full text to avoid indexing repeated queries and
retrieved snippets. Events provide structural filters;
`Finished`, `CompactionSummaryFailed` and `CompactionTranslationFailed` with a `Failed` outcome
additionally index their error text. Metadata is not part of full-text search.
