...
| GoogleGenerateContent | replace `:generateContent` with `:countTokens` | nested `generateContentRequest`, including model, system and tools |
...
to OpenAI counting; Claude's content cache markers are retained.

Only platform Responses, official Messages, and Gemini GenerateContent pairings are currently
supported. Chat Completions, Codex, Vertex, Bedrock and vendor-specific Messages modes are not
implicitly mapped to these endpoints. A compatible provider must explicitly expose the selected
...
and error handling. No automatic fallback to local estimation or per-turn count request is made.

Compaction policy remains a caller concern. Count against the same model and request that will be
used for generation; counts are not additive across messages and may differ from later usage.
